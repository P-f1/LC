package amqppublish

import (
	"fmt"

	"github.com/P-f1/LC/flogo-lib/core/activity"
	"github.com/P-f1/LC/flogo-lib/logger"
	"github.com/streadway/amqp"
)

const (
	ivUri          = "uri"
	ivExchangeName = "exchangeName"
	ivExchangeType = "exchangeType"
	ivRoutingKey   = "routingKey"
	ivBody         = "body"
	ivReliable     = "reliable"

	ovResult = "result"
)

var log = logger.GetLogger("activity-tibco-amqp")

// MyActivity is a stub for your Activity implementation
type MyActivity struct {
	metadata *activity.Metadata
}

// NewActivity creates a new activity
func NewActivity(metadata *activity.Metadata) activity.Activity {
	return &MyActivity{metadata: metadata}
}

// Metadata implements activity.Activity.Metadata
func (a *MyActivity) Metadata() *activity.Metadata {
	return a.metadata
}

func publish(amqpURI, exchange, exchangeType, routingKey, body string, reliable bool) error {

	// This function dials, connects, declares, publishes, and tears down,
	// all in one go. In a real service, you probably want to maintain a
	// long-lived connection as state, and publish against that.

	log.Debug("dialing %q", amqpURI)
	connection, err := amqp.Dial(amqpURI)
	if err != nil {
		return fmt.Errorf("Dial: %s", err)
	}
	defer connection.Close()

	log.Debug("got Connection, getting Channel")
	channel, err := connection.Channel()
	if err != nil {
		return fmt.Errorf("Channel: %s", err)
	}

	log.Debug("got Channel, declaring %q Exchange (%q)", exchangeType, exchange)
	if err := channel.ExchangeDeclare(
		exchange,     // name
		exchangeType, // type
		true,         // durable
		false,        // auto-deleted
		false,        // internal
		false,        // noWait
		nil,          // arguments
	); err != nil {
		return fmt.Errorf("Exchange Declare: %s", err)
	}

	// Reliable publisher confirms require confirm.select support from the
	// connection.
	if reliable {
		log.Debug("enabling publishing confirms.")
		if err := channel.Confirm(false); err != nil {
			return fmt.Errorf("Channel could not be put into confirm mode: %s", err)
		}

		confirms := channel.NotifyPublish(make(chan amqp.Confirmation, 1))

		defer confirmOne(confirms)
	}

	log.Debug("declared Exchange, publishing %dB body (%q)", len(body), body)
	if err = channel.Publish(
		exchange,   // publish to an exchange
		routingKey, // routing to 0 or more queues
		false,      // mandatory
		false,      // immediate
		amqp.Publishing{
			Headers:         amqp.Table{},
			ContentType:     "text/plain",
			ContentEncoding: "",
			Body:            []byte(body),
			DeliveryMode:    amqp.Transient, // 1=non-persistent, 2=persistent
			Priority:        0,              // 0-9
			// a bunch of application/implementation-specific fields
		},
	); err != nil {
		return fmt.Errorf("Exchange Publish: %s", err)
	}

	return nil
}

// One would typically keep a channel of publishings, a sequence number, and a
// set of unacknowledged sequence numbers and loop until the publishing channel
// is closed.
func confirmOne(confirms <-chan amqp.Confirmation) {
	log.Debug("waiting for confirmation of one publishing")

	if confirmed := <-confirms; confirmed.Ack {
		log.Debug("confirmed delivery with delivery tag: %d", confirmed.DeliveryTag)
	} else {
		log.Debug("failed delivery of delivery tag: %d", confirmed.DeliveryTag)
	}
}

// Eval implements activity.Activity.Eval
func (a *MyActivity) Eval(context activity.Context) (done bool, err error) {

	uri := context.GetInput(ivUri).(string)
	exchangeName := context.GetInput(ivExchangeName).(string)
	exchangeType := context.GetInput(ivExchangeType).(string)
	routingKey := context.GetInput(ivRoutingKey).(string)
	body := context.GetInput(ivBody).(string)
	reliable := context.GetInput(ivReliable).(bool)

	err = publish(uri, exchangeName, exchangeType, routingKey, body, reliable)

	if err != nil {
		return false, err
	}

	context.SetOutput(ovResult, "OK")
	return true, nil
}
