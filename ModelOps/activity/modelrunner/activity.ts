...
@WiContrib({})
@Injectable()
export class ModelRunnerContributionHandler extends WiServiceHandlerContribution {
	filename: string;
	content: string; 
    ...
    value = (fieldName: string, context: IActivityContribution): Observable<any> | any => {
		...
		if (fieldName === "DataIn") {			
            var attrJsonSchema = {};
            let attrNames: IFieldDefinition = context.getField("InputFields");
            if (attrNames.value) {
        ...
        } else if (fieldName === "DataOut") {			
            var attrJsonSchema = {};
            let attrNames: IFieldDefinition = context.getField("OutputFields");
            if (attrNames.value) {
                let data = JSON.parse(attrNames.value);
                for (var i = 0; i < data.length; i++) {
                		attrJsonSchema[data[i].Name] = populateAttribute(data[i].Type);
