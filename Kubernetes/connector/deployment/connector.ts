...
@WiContrib({})
@Injectable()
export class KubernetesDeployment extends WiServiceHandlerContribution {
    constructor() {
        super();
    }
	...
	validate = (name: string, context: IConnectorContribution): Observable<IValidationResult> | IValidationResult => {
		...
		if(name === "Connect") {
			let template: IFieldDefinition;
			let filename: string;
			let content: string; 

			for (let configuration of context.settings) {
				if( configuration.name === "template") {
	...
	action = (actionName: string, context: IConnectorContribution): Observable<IActionResult> | IActionResult => {
		...
		if (actionName == "Connect") {
			...
			console.log('------------- action callback --------------');
                 ...
                 */
                let deloyment: IFieldDefinition;
 
                for (let configuration of context.settings) {
                    if (configuration.name === "template") {
                        deloyment = configuration
						console.log(deloyment.value);
                    }
                }
