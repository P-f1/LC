...
func (a *BuilderActivity) Eval(context activity.Context) (done bool, err error) {
	...
	log.Info("[BuilderActivity:Eval] Nodes : ", context.GetInput(Nodes))
	log.Info("[BuilderActivity:Eval] Edges : ", context.GetInput(Edges))
	...
	err = a.graphBuilder.BuildGraph(
		&deltaGraph,
		graphModel,
		context.GetInput(Nodes),
		context.GetInput(Edges),
		allowNullKey.(bool),
	)
	...
	if nil == context.GetInput(BatchEnd) || context.GetInput(BatchEnd).(bool) {
		...
		if 0 != len(passThroughDataDef) {
			log.Info("[BuilderActivity:Eval] PassThroughData : ", context.GetInput(PassThroughData))
