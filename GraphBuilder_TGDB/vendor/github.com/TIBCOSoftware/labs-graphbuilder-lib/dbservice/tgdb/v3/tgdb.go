...
func (this *TGDBService) ensureConnection() tgdb.TGError {
	...
	if nil == this._tgConnection {
		logger.Info("XXXXXXXXXXXXXXXXXXXXXX Start connecting XXXXXXXXXXXXXXXXXXXXXXXXXXXXXX")
		logger.Info("[TGDBService::ensureConnection] Will try to connect ..........")
		logger.Info(fmt.Sprintf("[TGDBService::ensureConnection] url = %s ", this._url))
		logger.Info(fmt.Sprintf("[TGDBService::ensureConnection] user = %s ", this._user))
		logger.Info(fmt.Sprintf("[TGDBService::ensureConnection] password = %s ", this._password))
		logger.Info(fmt.Sprintf("[TGDBService::ensureConnection] connectionProps = %s ", this._connectionProps))
		defer logger.Info("XXXXXXXXXXXXXXXXXXXXXX End connecting XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX")
		...
		if nil != err {
			...
			logger.Error(fmt.Sprintf("[TGDBService::ensureConnection] ERROR !!! unable to create connection : error =  ", err.GetErrorMsg()))
			return err
		}
		...
		if nil != err {
			this._tgConnection = nil
			this._gof = nil
			logger.Error(fmt.Sprintf("[TGDBService::ensureConnection] ERROR !!! unable to connection to TGDB server : error =  ", err.GetErrorMsg()))
			return err
		}

		err := this.fetchMetadata()
		if nil != err {
			logger.Error(fmt.Sprintf("[TGDBService::ensureConnection] ERROR !!! unable to get metadata : error =  ", err.GetErrorMsg()))
			return err
		}
		...
		if nil != err {
			this._tgConnection = nil
			logger.Error(fmt.Sprintf("[TGDBService::ensureConnection] ERROR !!! unable to get object factory : error =  ", err.GetErrorMsg()))
			return err
		}
